...
**Reason**: 原要求同时规定 LateUpdate 同步及 Look 不经适配器，两者分别与当前运动消费时序、InvokeCSharpEvents 下的实际 Look 入口不一致；无条件覆盖 Jump 还会重新注入已被消费的请求。用下述完整输入适配契约替代，不删除运动适配能力。
**Migration**: 保留 SAInputAdapter 作为项目适配层，采用“StarterAssets 输入消费与 Look 直通”的行为；原“SAInputAdapter 同步生效”继续覆盖，原“look 字段不经 SAInputAdapter 写入”的断言明确废止并由唯一写入者验收替代。
