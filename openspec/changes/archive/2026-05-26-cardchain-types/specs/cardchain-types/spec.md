...
`CardChainCore/tests/Unomata.Core.Tests/` 下 SHALL 存在覆盖本 change 所有 Scenario 的 xUnit 测试。`dotnet test CardChainCore.sln` SHALL 退出码为 0，且所有新增测试用例 SHALL 通过。

#### Scenario: 测试套件可运行且全部通过

- **WHEN** 在 `CardChainCore/` 下执行 `dotnet test CardChainCore.sln`
- **THEN** 退出码 SHALL 为 0，且输出 SHALL 报告本 change 引入的测试全部通过、零失败、零跳过
